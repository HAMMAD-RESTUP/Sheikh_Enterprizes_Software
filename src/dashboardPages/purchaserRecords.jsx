...
export default function PurchaserRecords() {
  ...
  const rows = useMemo(() => {
    const onlyPurchase = (Array.isArray(list) ? list : []).filter(
      (t) => String(t?.type || "").toLowerCase() === "purchase"
    );

    onlyPurchase.sort((a, b) => {
    ...
    if (!term) return onlyPurchase;

    return onlyPurchase.filter((t) => {
      const party = (t.partyName || t.customerName || t.sellerName || "").toLowerCase();
      const inv = String(t.invoiceNo || "").toLowerCase();
      return party.includes(term) || inv.includes(term) || "purchase".includes(term);
    });
  }, [list, q]);
